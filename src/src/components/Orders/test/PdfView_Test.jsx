...
export const PdfView_Test = () => {
  ...
  const pdfAllData = () => {
    axios
      .post(`${API_BASE_URL}/NewOrderPdfDetails`, {
  ...
  const generatePdf = async () => {
    ...
    const addLogoWithDetails = async () => {
      ...
      const textDataLeft = [
        { label: "Order :", value: `${data1?.Order_Number}` },
        { label: "TT Ref :", value: `${data1.Shipment_ref}` },
      ...
      const textDataRight = [
        { label: "Date:", value: `${formatDate(data1.created)}` },
  ...
  const uploadPDF = async (pdfBlob) => {
    ...
    formData.append(
      "document",
      pdfBlob,
      `${from?.Order_Number || "default"}_Operation_Test_${dateTime}.pdf`
    );
    setIsLoading(true);
    loadingModal.fire();
    try {
      ...
      if (response.data.success) {
        ...
        window.open(
          `${API_IMAGE_URL}${from?.Order_Number}_Operation_Test_${dateTime}.pdf`
        );
      } else {
