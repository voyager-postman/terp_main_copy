...
const QuotationTest = () => {
  ...
  //       `${API_BASE_URL}/Quotation_Status_1_2`,
  //       {
  //         order_id: "John Doe",
  //         user_id: "john@example.com",
  //       }
  //     );
  //     toast.success(t("quotationUpdateSuccess"), {
  //       position: "top-right",
  //     });
  //     console.log("statusTrue:", response.data);
  //   } catch (error) {
  ...
  //   }
  // };
  const updateQuotationStatus = async (order_id) => {
    console.log("Sending payload:", { order_id }); // 争 check values here
        ...
        dynamicColumns.push({
          ...
          accessor: (a) => (
            <div className="editIcon">
              ...
              {[1, 2].includes(+a.Status_value) && (
                <button
                  type="button"
                  onClick={() => updateQuotationStatus(a.Order_ID)}
                >
                  <i className="mdi mdi-check" />
                </button>
              )}

              {+a.Status_value >= 3 && +a.Status_value <= 13 && (
                <button
                  type="button"
              ...
              {+a.Status_value >= 3 && +a.Status_value <= 15 && (
                <button
                  type="button"
                  style={{
                    width: "20px",
  ...
  const performaOrder = async (a) => {
    try {
      ...
      const addLogoWithDetails = async () => {
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(
          `${invoiceResponse.data.Section1_Labels.Row1} ${invoiceResponse.data.section1_Values.Row1
        ...
        doc.text(
          `${invoiceResponse.data.Section1_Labels.Row2} ${invoiceResponse.data.section1_Values.Row2
        ...
        doc.text(
          `${invoiceResponse.data.Section1_Labels.Row3} ${invoiceResponse.data.section1_Values.Row3
        ...
        doc.text(
          `${invoiceResponse.data.Section1_Labels.Row4} ${invoiceResponse.data.section1_Values.Row4
      ...
      doc.text(
        `${invoiceResponse.data.section6_Labels.Row1} ${invoiceResponse.data.section6_Values.Row1
      ...
      doc.text(
        `${invoiceResponse.data.section6_Labels.Row2} ${invoiceResponse.data.section6_Values.Row2
      ...
      doc.text(
        `${invoiceResponse.data.section6_Labels.Row3} ${invoiceResponse.data.section6_Values.Row3
      ...
      // 隼 Step 1: Render longText first (if exists)
      ...
      if (inputFieldValue && inputFieldValue.trim() !== "") {
        ...
        // 隼 Only draw line if longText was present
        if (hasLongText) {
