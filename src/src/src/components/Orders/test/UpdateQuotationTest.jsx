...
const UpdateQuotationTest = () => {
      ...
      .then((response) => {
        ...
        if (response.data?.data) {
          ...
          setState((prevState) => ({
            ...
            Q_Markup: newData.Q_Markup || prevState.Q_Markup,
  ...
  const computedState = useMemo(() => {
    ...
    r.mark_up = r.mark_up || consigneeFind?.Q_Markup;
  ...
  const handleAgreedPricingChange4 = async (e) => {
    ...
    setExchangeRate1(newValue); // Update local state

    try {
      const response = await updateAllOrderStatuses({
        id: state?.order_id, // Safe access
        field: name,
        value: newValue,
      });

      // Optional: Check for response success before showing toast
      if (response?.data?.message) {
        toast.success(response.data.message, {
          autoClose: 1000,
          theme: "colored",
  ...
  //   setExchangeRate4(e.target.checked);
  //   console.log(exchangeRate4);
  // };
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/InvoicePriceRounding`,
        {
          ...
          Is_Invoice: 0,
          Is_Quotation: newdata?.Is_quotation,
          Is_Recalculate: 0,
        }
      );
      console.log(response);
 ...
 `,
  ...
  const reCalculate = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        oneQoutationDAta();
        console.log(response);
        toast.success(t("quotationRecalculatedSuccess"), {
          autoClose: 1000,
          theme: "colored",
        });
      })
  ...
  return (
    <>
      <Card title={`${t("quotationTestManagement")} / ${t("updateForm")}`}>
                  ...
                  <form action="">
                    ...
                    <div className="row formEan quotationRowDro">
                      ...
                      <div className="col-lg-3 form-group mb-3 quotationSelectSer">
                        ...
                        <Autocomplete
                          ...
                          sx={{ width: 300 }}
                          getOptionLabel={(option) => option?.name || ""} // Display name of the brand
                          isOptionEqualToValue={(option, value) =>
                            option.id === value?.id
                          } // Compare by ID
                          value={
                            brands?.find(
                              (brand) => brand.ID === computedState.brand_id
                            )
                              ? {
                                id: computedState.brand_id,
                                name: brands.find(
                                  (brand) =>
                    ...
                    <div className="flex gap-2 items-center justify-between flex-wrap">
                      ...
                      <div className="addBtnEan mb-4">
                        <button type="button" className="mt-0" onClick={() => calculate(true)}>
                          {t("recalculate")}
                        </button>
                      </div>
                    </div>
...
export default UpdateQuotationTest;
