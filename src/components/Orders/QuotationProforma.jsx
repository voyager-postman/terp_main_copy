...
//       doc.setFontSize(10);
//       doc.setTextColor(0, 0, 0);
//       doc.text(`Order: ${from?.Quotation_number}`, 127, 12);
//       doc.text(`TT Ref: `, 127, 16.5);
//       doc.text(`Loading Date: ${formatDate(data.load_Before_date)}`, 127, 20);
...
//       calculated_price: `${formatterNg.format(
//         item.quotation_price ? item.quotation_price : item.calculated_price
//       )}`,
//       line_total: newFormatter3.format(
//         item.itf_quantity *
//           (item.quotation_price ? item.quotation_price : item.calculated_price)
//       ),
//     }));
...
//     doc.text(
//       `Total Box: ${formatterNo.format(totalDetails?.box)}`,
//       7,
//       finalY + 1
//     );
//     doc.text(`Total Items: ${totalDetails?.Items}`, 7, finalY + 5);
//     doc.text(
//       `Total Net Weight: ${formatterNg.format(totalDetails?.nw)}`,
//       75,
//       finalY + 5
//     );
//     doc.text(
//       `Total Gross Weight: ${formatterNo.format(totalDetails?.gw)}`,
//       75,
//       finalY + 9
//     );
//     doc.text(`Total CBM: ${totalDetails?.cbm}`, 75, finalY + 1);
...
//     const label = "Total: ";
//     const value = `${formatter5.format(totalDetails?.CNF_FX)}`;
...
const QuotationProforma = () => {
  ...
  const generatePdf = async () => {
    ...
    const addLogoWithDetails = async () => {
      ...
      doc.text(`Order: ${from?.Quotation_number}`, 127, 12);
      doc.text(`TT Ref: `, 127, 16.5);
      doc.text(`Loading Date: ${data.load_Before_date}`, 127, 20);
      doc.text(`Delivery By: ${messageSet1}`, 127, 24.5);
    };
    doc.setFillColor(32, 55, 100);
    doc.rect(7, 27, doc.internal.pageSize.width - 15, 0.5, "FD");
    ...
    const rows = tableData.map((item, index) => ({
      index: index + 1,
      HS_CODE: item.HS_CODE,
      net_weight: `${formatterNg.format(item.QOD_NW)}`,
      Number_of_boxes: `${formatterNo.format(item.QOD_Box)}`,
      itf_name_en: item.itf_name_en,
      itf_quantity: `${formatterNg.format(item.QOD_QTY)}`,
      unit_name_en: item.unit_name_en,
      calculated_price: `${twoDecimal.format(
        item.QOD_QP ? item.QOD_QP : item.QOD_CP
      )}`,
      line_total: twoDecimal.format(
        item.QOD_QTY * (item.QOD_QP ? item.QOD_QP : item.QOD_CP)
      ),
    }));
    doc.autoTable({
      head: [columns.map((col) => col.header)],
    ...
    doc.text(
      `Total Box: ${formatterNo.format(totalDetails?.Q_Box)}`,
      7,
      finalY + 1
    );
    doc.text(
      `Total Items: ${formatterNo.format(totalDetails?.Q_Items)}`,
      7,
      finalY + 5
    );
    doc.text(
      `Total Net Weight: ${threeDecimal.format(totalDetails?.Q_NW)}`,
      7,
      finalY + 9
    );
    // doc.text(
    ...
    const value = `${twoDecimal.format(totalDetails?.Q_CNF_FX)}`;
  ...
  const uploadPDF = async (pdfBlob) => {
    ...
    formData.append(
      "document",
      pdfBlob,
      `${from?.Quotation_number || "default"}_Proforma_${dateTime}.pdf`
    );
    try {
      const response = await axios.post(`${API_BASE_URL}/UploadPdf`, formData);
      console.log(response);
      if (response.data.success) {
        console.log("PDF uploaded successfully");
        window.open(
          `${API_IMAGE_URL}${from?.Quotation_number}_Proforma_${dateTime}.pdf`
        );
      } else {
        console.log("Failed to upload PDF");
      }
