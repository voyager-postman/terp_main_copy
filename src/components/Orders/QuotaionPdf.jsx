...
//       doc.text(`${formatterNg.format(totalDetails[0]?.Q_NW)}`, 117, 28);
...
//       doc.setTextColor(0, 0, 0);
//     };
//     function renderWrappedText(doc, text, startX, startY, maxWidth, lineHeight) {
...
//     currentY1 = renderWrappedText(doc, longText1_1, startX1, currentY1, maxWidth1, lineHeight1);
//     doc.setFontSize(10);
//     currentY1 = renderWrappedText(doc, longText1_2, startX1, currentY1, maxWidth1, lineHeight1);
...
//     currentY2 = renderWrappedText(doc, longText2_1, startX2, currentY2, maxWidth2, lineHeight1);
//     doc.setFontSize(10);
//     currentY2 = renderWrappedText(doc, longText2_2, startX2, currentY2, maxWidth2, lineHeight1);
//     currentY2 = renderWrappedText(doc, longText2_3, startX2, currentY2, maxWidth2, lineHeight1);
...
//       quotation_price_unit: `${twoDecimal.format(item.quotation_price)}    ${item.Unit_price
...
//       "document",
//       pdfBlob,
//       `${from?.Quotation_number || "default"}_Quotation_${dateTime}.pdf`
//     );
//     setIsLoading(true);
//     loadingModal.fire();
//     try {
...
//         window.open(
//           `${API_IMAGE_URL}${from?.Quotation_number}_Quotation_${dateTime}.pdf`
//         );
//       } else {
...
const ProformaInvoice = () => {
  ...
  const generatePdf = async () => {
    ...
    const rows = tableData.map((item) => ({
      ...
      ITF_Scientific_name: item.ITF_Scientific_name,
      ITF_HSCODE: item.ITF_HSCODE,
      quotation_price_unit: `${twoDecimal.format(item.quotation_price)} ${
        item.Unit_price
      }`,
    }));

    let tableStartY = Math.max(currentY1, currentY2) || 10;

    const renderTable = (rowsData) => {
      const header = includeImage
        ? [["Item", "", "", " ", " "]]
        : [["Item", "Scientific Name", "Hs Code", "Price & Unit"]];
      const body = rowsData.map((row) =>
        includeImage
          ? [
              `${row.ITF_Name.trim()}\n${row.ITF_Scientific_name.trim()}\n${row.ITF_HSCODE.trim()}\n${row.quotation_price_unit.trim()}`,
              "",
              "",
              "",
              "",
      ...
      doc.autoTable({
        ...
        startY: tableStartY,
        margin: {
          top: 10, // New page top margin will be 10
          left: 7,
          right: 7,
        },
        didDrawPage: (data) => {
          tableStartY = data.settings.margin.top; // Reset y-axis for new page
        },
        willDrawCell: (data) => {
          if (data.section === "body") {
            const contentHeight = doc.getTextDimensions(data.cell.raw).h;
            ...
            // (Optional) Fill background if needed for non-image rows
            if (!includeImage && data.row.index % 2 === 0) {
              doc.setFillColor(242, 242, 242);
              doc.rect(
                data.cell.x,
        ...
        didDrawCell: (data) => {
          if (data.section === "body") {
            const padding = 0;
            if (includeImage && [1, 2, 3, 4].includes(data.column.index)) {
              const img = new Image();
              img.src = imageUrl;
              const imgX = data.cell.x + padding;
              const imgY = data.cell.y + padding;
              const imgWidthScaled = data.cell.width; // Fit to column
              const imgHeightScaled = imgHeight * (imgWidthScaled / imgWidth); // Maintain aspect ratio
              doc.addImage(
                img,
                "JPEG",
                imgX,
              ...
              // Update cell and row height based on image dimensions
              const bottomPadding = 0;
              const newHeight =
                imgY + imgHeightScaled - data.cell.y + bottomPadding;
              data.cell.height = Math.max(newHeight, data.cell.height);
              data.row.height = Math.max(data.row.height, newHeight);
            } else {
              // For cells without an image, the border is drawn later in didDrawRow
            }
          }
        },
        didDrawRow: (rowData) => {
          const rowY = rowData.row.y;
          const rowHeight = rowData.row.height;
          const marginLeft = rowData.settings.margin.left;
          const tableWidth = rowData.table.width;
          const extraPadding = 5; // Space after image row
          ...
          if (includeImage) {
            ...
            // Add space after the image row
            rowData.row.height += extraPadding;
          } else {
            // Set border color to default (dark blue)
            doc.setDrawColor(32, 55, 100);
            doc.setLineWidth(1);

            // Draw full border for non-image rows
            doc.rect(marginLeft, rowY, tableWidth, rowHeight, "S");
          }
        },

        columnStyles: {
          0: { halign: includeImage ? "left" : "left", cellWidth: 59 },
          1: {
            halign: includeImage ? "left" : "center",
            cellWidth: includeImage ? 35 : 70,
          },
          2: {
            halign: includeImage ? "left" : "center",
            cellWidth: includeImage ? 35 : 30,
          },
          3: {
            halign: includeImage ? "left" : "center",
            cellWidth: includeImage ? 35 : 37,
          },
          4: {
            halign: includeImage ? "left" : "center",
            cellWidth: includeImage ? 35 : 47,
          },
        },
        headStyles: {
          fillColor: [32, 55, 100],
    ...
    // If exchangeRate is true, split the first three rows for the first page and remaining rows for subsequent pages
    if (exchangeRate) {
      const firstPageRows = rows.slice(0, 5); // First three rows for the first page
      const subsequentRows = rows.slice(3); // Remaining rows for the subsequent pages
      ...
      // If `exchangeRate` is true, split the remaining rows across subsequent pages
      if (subsequentRows.length > 0) {
        const rowsPerPage = 7; // Set to 4 rows per page for subsequent pages
        let currentIndex = 0;
