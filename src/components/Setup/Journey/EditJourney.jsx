import axios from "axios";
import { useState, useEffect } from "react";
import { Link, useNavigate, useLocation } from "react-router-dom";
import { toast } from "react-toastify";
import { API_BASE_URL } from "../../../Url/Url";
import { Card } from "../../../card";
import { useQuery } from "react-query";
import Autocomplete from '@mui/material/Autocomplete';
import TextField from '@mui/material/TextField';
const EditJourney = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { from } = location.state || {};
  console.log(from);

  const { data: ports } = useQuery("getAllAirports");
  const { data: liner } = useQuery("getLiner");

  const defaultState = {
    from_port_id: 0,
    destination_port_id: 0,
    liner_id: 0,
    journey_number: "",
    currency: "",
    load_time: "",
    transit_to_departure: "",
    etd: "",
    transit_to_arrival: "",
    eta: "",
  };

  const [state, setState] = useState(defaultState);

  useEffect(() => {
    if (from) {
      setState((prevState) => ({
        ...prevState,
        from_port_id: from?.from_port_id,
        destination_port_id: from?.asdestination_port_id,
        liner_id: from?.Liner_ID,
        journey_number: from?.journey_number,
        currency: from?.currency,
        load_time: from?.Load_time,
        transit_to_departure: from?.Transit_to_Departure
        ,
        etd: from?.ETD,
        transit_to_arrival: from?.Transit_to_arrival,
        eta: from?.ETA,
      }));
    }
  }, [from]);

  const handleChange = (event) => {
    const { name, value } = event.target;
    setState((prevState) => ({
      ...prevState,
      [name]: value,
    }));
  };

  const addBank = () => {
    const request = {
      journey_id: from?.ID,
      from_port: state.from_port_id,
      destination_port: state.destination_port_id,
      liner_id: state.liner_id,
      journey_number: state.journey_number,
      load_time: state.load_time,
      Transit_to_Departure: state.transit_to_departure,
      ETD: state.etd,
      Transit_to_arrival: state.transit_to_arrival,
      ETA: state.eta,
    };

    axios
      .post(`${API_BASE_URL}/Addjourneydetails`, request)
      .then((response) => {
        console.log(response, "Check responseee");
        if (response.data.success == true) {
          toast.success("Journey Update Successfully", {
            autoClose: 1000,
            theme: "colored",
          });
          navigate("/journey");
          return;
        }
      })
      .catch((error) => {
        console.log(error);
      });
  };

  return (
    <Card title="Journey / Create Form">
      <div className="top-space-search-reslute">
        <div className="tab-content px-2 md:!px-4">
          <div className="tab-pane active" id="header" role="tabpanel">
            <div
              id="datatable_wrapper"
              className="information_dataTables dataTables_wrapper dt-bootstrap4"
            >
              <div className="formCreate">
                <form action="">
                  <div className="row">
                    <div className="form-group col-lg-3 autoComplete">
                      <h6>From Port</h6>
                      {/* <select
                        name="from_port_id"
                        onChange={handleChange}
                        value={state.from_port_id}
                      >
                        <option value={0}>Please select From </option>
                        {ports?.map((item) => (
                          <option
                            key={item.port_id}
                            value={item.port_id}
                          >
                            {item.port_name}
                          </option>
                        ))}
                      </select> */}
                      <Autocomplete
                        options={
                          ports?.map((item) => ({
                            id: item.port_id,
                            name: item.port_name,
                          })) || []
                        } // Map ports to create options with `id` and `name`
                        getOptionLabel={(option) => option.name || ""} // Display the port name
                        value={
                          ports
                            ?.map((item) => ({
                              id: item.port_id,
                              name: item.port_name,
                            }))
                            .find((option) => option.id === state.from_port_id) ||
                          null
                        } // Match the current `from_port_id` in state with the options
                        onChange={(e, newValue) => {
                          setState({ ...state, from_port_id: newValue?.id || 0 }); // Update state with selected port's `id`
                        }}
                        isOptionEqualToValue={(option, value) =>
                          option.id === value.id
                        } // Ensure proper option matching
                        sx={{ width: 300 }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            placeholder="Select From Port" // Adds a placeholder
                            InputLabelProps={{ shrink: false }} // Prevents floating label
                          />
                        )}
                      />
                    </div>
                    <div className="form-group col-lg-3 autoComplete">
                      <h6>Designation Port</h6>
                      {/* <select
                        name="destination_port_id"
                        onChange={handleChange}
                        value={state.destination_port_id}
                      >
                        <option value={0}>Please select Designation</option>
                        {ports?.map((item) => (
                          <option
                            key={item.port_id}
                            value={item.port_id}
                          >
                            {item.port_name}
                          </option>
                        ))}
                      </select> */}
                      <Autocomplete
                        options={
                          ports?.map((item) => ({
                            id: item.port_id,
                            name: item.port_name,
                          })) || []
                        } // Map ports to create options with `id` and `name`
                        getOptionLabel={(option) => option.name || ""} // Display the port name
                        value={
                          ports
                            ?.map((item) => ({
                              id: item.port_id,
                              name: item.port_name,
                            }))
                            .find((option) => option.id === state.destination_port_id) ||
                          null
                        } // Match the current `destination_port_id` in state with the options
                        onChange={(e, newValue) => {
                          setState({ ...state, destination_port_id: newValue?.id || 0 }); // Update state with selected port's `id`
                        }}
                        isOptionEqualToValue={(option, value) =>
                          option.id === value.id
                        } // Ensure proper option matching
                        sx={{ width: 300 }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            placeholder="Select Destination Port" // Adds a placeholder
                            InputLabelProps={{ shrink: false }} // Prevents floating label
                          />
                        )}
                      />
                    </div>
                    <div className="form-group col-lg-3 autoComplete">
                      <h6> Liner </h6>
                      {/* <select
                        name="liner_id"
                        onChange={handleChange}
                        value={state.liner_id}
                      >
                        <option value={0}>Please select Liner</option>
                        {liner?.map((item) => (
                          <option
                            key={item.liner_id}
                            value={item.liner_id}
                          >
                            {item.liner_name}
                          </option>
                        ))}
                      </select> */}
                      <Autocomplete
                        options={
                          liner?.map((item) => ({
                            id: item.liner_id,
                            name: item.liner_name,
                          })) || []
                        } // Map liner to create options with `id` and `name`
                        getOptionLabel={(option) => option.name || ""} // Display the liner name
                        value={
                          liner
                            ?.map((item) => ({
                              id: item.liner_id,
                              name: item.liner_name,
                            }))
                            .find((option) => option.id === state.liner_id) ||
                          null
                        } // Match the current `liner_id` in state with the options
                        onChange={(e, newValue) => {
                          setState({ ...state, liner_id: newValue?.id || 0 }); // Update state with selected liner's `id`
                        }}
                        isOptionEqualToValue={(option, value) =>
                          option.id === value.id
                        } // Ensure proper option matching
                        sx={{ width: 300 }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            placeholder="Select Liner" // Adds a placeholder
                            InputLabelProps={{ shrink: false }} // Prevents floating label
                          />
                        )}
                      />
                    </div>
                    <div className="form-group col-lg-3">
                      <h6>Journey Number</h6>
                      <input
                        name="journey_number"
                        onChange={handleChange}
                        value={state.journey_number}
                        type="text"
                      />
                    </div>
                  </div>
                  <div className="row">
                    <div className="form-group col-lg-4">
                      <h6>Load Time</h6>
                      <input
                        onChange={handleChange}
                        type="time"
                        id="name_en"
                        name="load_time"
                        value={state.load_time}
                      />
                    </div>

                    <div className="form-group col-lg-4">
                      <h6>Transit to Departure</h6>
                      <input
                        onChange={handleChange}
                        type="number"
                        id="name_en"
                        name="transit_to_departure"
                        className="form-control"
                        placeholder="Transit to Departure"
                        value={state.transit_to_departure}
                      />
                    </div>

                    <div className="form-group col-lg-4">
                      <h6>ETD</h6>
                      <input
                        onChange={handleChange}
                        type="time"
                        id="name_en"
                        name="etd"
                        value={state.etd}
                      />
                    </div>
                    <div className="form-group col-lg-6">
                      <h6>Transit to Arrival</h6>
                      <input
                        onChange={handleChange}
                        type="number"
                        id="name_en"
                        name="transit_to_arrival"
                        className="form-control"
                        placeholder="Transit to Arrival"
                        value={state.transit_to_arrival}
                      />
                    </div>
                    <div className="form-group col-lg-6">
                      <h6>ETA</h6>
                      <input
                        onChange={handleChange}
                        type="time"
                        id="name_en"
                        name="eta"
                        value={state.eta}
                      />
                    </div>
                  </div>
                </form>
              </div>
            </div>
          </div>
          <div className="card-footer">
            <button
              onClick={addBank}
              className="btn btn-primary"
              type="submit"
              name="signup"
            >
              Update
            </button>
            <Link className="btn btn-danger" to="/journey">
              Cancel
            </Link>
          </div>
        </div>
      </div>
    </Card>
  );
};

export default EditJourney;
