...
const optionsEmp = [
  { label: "black rasgulla", value: "apple" },
  { label: "Banana", value: "banana" },
  { label: "Cherry", value: "cherry" },
  { label: "Date", value: "date" },
];
const AdvancePayment = () => {
  ...
  const getAllAdvancePayment = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}GetAllAdvancePayments`);
      console.log("Vacation data:", response.data.data);
      setAdvancePayment(response.data.data);
...
GetTransactionType`
  ...
  const data = [
    {
      emp: "36 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Annual",
      leaveDate: "13/2024",
      leaveDays: 2,
    },
    {
      emp: "143 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Sick",
      leaveDate: "13/2024",
      leaveDays: 3,
    },
    {
      emp: "54 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Sick",
      leaveDate: "13/2024",
      leaveDays: 12,
    },
    {
      emp: "134 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Annual",
      leaveDate: "13/2024",
      leaveDays: 4,
    },
    {
      emp: "456 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Sick",
      leaveDate: "13/2024",
      leaveDays: 8,
    },
    {
      emp: "167: อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Annual",
      leaveDate: "13/2024",
      leaveDays: 7,
    },
    {
      emp: "256 : อ้วน พื้นชนะ (อ้วน)	",
      leaveType: "Annual",
      leaveDate: "13/2024",
      leaveDays: 12,
    },
  ...
  const handlepaymentEmp = (e) => SetEmpDetails(e.target.value);
  // Filter data based on search input
  const filteredData = advancePayment.filter(
    (row) =>
      ...
      (row.transaction_date &&
        formatDate(row.transaction_date).includes(search)) ||
      (row.amount && row.amount.toString().includes(search)) ||
      (row.transaction_type &&
        row.transaction_type.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...
        transaction_id: formData.leave_type,
        transaction_date: moment(formData.startDate).format("YYYY-MM-DD"),
        amount: formData.quantity,
      };

      // Send the payload to the API
      const response = await axios.post(
        `${API_BASE_URL}AddAdvancePayment`,
        payload
      );
      ...
      Swal.fire({
        title: "Success!",
        text: "Advance Payment created successfully!",
        icon: "success",
      });
    } catch (error) {
  ...
  const defaultStateSet = (row) => {
    ...
    setFormData({
      ...
      leave_type: row.transaction_id || "",
      startDate:
        new Date(row.transaction_date).toISOString().split("T")[0] || "",
      quantity: row.amount || "",

      AdvancePayment_id: row.advancePayment_id || "",
    });
  };
  const updateSubmitData = async () => {
    try {
      // Prepare the payload
      const payload = {
        advancePayment_id: formData.AdvancePayment_id,
        employee_id: formData.employee,
        transaction_id: formData.leave_type,
        transaction_date: moment(formData.startDate).format("YYYY-MM-DD"),
        amount: formData.quantity,
      ...
      // Send the payload to the API
      const response = await axios.post(
        `${API_BASE_URL}UpdateAdvancePayment`,
        payload
      );
  ...
  const deleteEmployee = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.post(
            `${API_BASE_URL}DeleteAdvancePayment`,
            {
              AdvancePayment_id: id,
            }
          );
          console.log(response);
          getAllAdvancePayment();
          toast.success("Advance Payment delete successfully");
        } catch (e) {
                    ...
                    <div className="modal-content">
                      <div className="modal-header">
                        <h1 className="modal-title fs-5" id="exampleModalLabel">
                          Advance Payments Management Create Form
                        </h1>
                        <button
                          type="button"
                          className="btn-close"
                            ...
                            <td>
                              <div className="editIcon">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    deleteEmployee(row.advancePayment_id)
                                  }
                                >
                                ...
                                <div className="modal-dialog modal-xl">
                                  <div className="modal-content">
                                    <div className="modal-header">
                                      ...
                                      >
                                        Advance Payments Management Edit Form
                                      </h1>
                                      <button
                                        type="button"
                                        className="btn-close"
