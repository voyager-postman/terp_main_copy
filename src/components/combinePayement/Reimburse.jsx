...
const CombinePaymentEdit = () => {
 ...
 =${idToUse}`
  ...
  return (
    <>
      <Card
        title={`${t("Reimburse")} / ${from?.ID ? t("update") : t("create")} ${t(
          "form"
        )}`}
      >
                      ...
                      <div className="col-lg-3 form-group autoComplete">
                        ...
                        <Autocomplete
                          ...
                          value={
                            recieptDroupDown
                              ?.filter((item) =>
                                state.vendor_ids?.includes(item.ID)
                              ) // ✅ match by ID
                              .map((item) => ({
                                id: item.ID,
                                name: item.Name,
                          ...
                          onChange={(e, newValue) => {
                            setState({
                              ...state,
                              vendor_ids: newValue.map((val) => val.id), // ✅ store IDs
                              vendor_names: newValue.map((val) => val.name), // optional: store names
                            });
                          }}
                          sx={{ width: 300 }}
                          renderInput={(params) => (
