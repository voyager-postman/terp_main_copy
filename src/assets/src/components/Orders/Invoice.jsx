...
const Invoice = () => {
  ...
  const lang = localStorage.getItem("language") || "en"; // fallback to "en"
  const langValue = lang === "en" ? 1 : 2;
  const allInvoiceData = () => {
    axios
      .get(`${API_BASE_URL}/invoiceDetailsList`, {
        params: { lang: langValue },
      })
      .then((res) => {
        setData(res.data.data || []);
        setDateLabels(res.data.dateLabels || {});
      })
      .catch((error) => {
  ...
  const columns = useMemo(() => {
    const dynamicCols = Object.keys(dateLabels).map((key) => ({
      Header: dateLabels[key], // e.g. "Invoice Date"
      accessor: key, // e.g. "Col1"
    }));
    ...
    dynamicCols.push({
          ...
          <div className="editIcon">
            ...
            <>
              ...
              {(+a.IC2 === 1) && (
                <button
                  type="button"
                  onClick={() => restoreEanPackage(a.ID)}
                >
      ...
      {[5, 1, 0].includes(parseInt(a.Col6.split("-")[0], 10)) && (
              <button className="rotateDown">
  ...
  useEffect(() => {
    if (details && Array.isArray(details)) {
      ...
      details.dateValues.forEach((item, i) => {
        initialPaidAmounts[i] = item.Col5 || ""; // Col5 is the paid amount
        initialAmounts[i] = item.Col6 || ""; // total amount
        initialUnits[i] = item.Col3 || ""; // example
        initialClaims[i] = ""; // default
      });
  ...
  const summaryTable = async (Claim_ID) => {
    ...
    const dataToSubmit = details.dateValues
      ...
      .map((item, i) => ({
        ...
        Order_ID: data1?.ID,
        OD_ID: item.ID,
        Claimed_Qty: parseFloat(paidAmounts[i]),
        Claimed_Unit: units[i],
        Claimed_Amount: parseFloat(amounts[i]),
        Claim_Reason: claims[i],
            ...
            <div className="modal-body">
              <div className="claimParent">
                <div>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result1 || "-"}
                  </span>
                </div>
                <div>
                  <strong>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result2 || "-"}
                  </span>
                </div>
                <div>
                  <strong>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result3 || "-"}
                  </span>
                </div>
                <div>
                  <strong>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result4 || "-"}
                  </span>
                </div>
                <div>
                  <strong>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result5 || "-"}
                  </span>
                </div>
                <div>
                  <strong>
                  ...
                  <span>
                    {details?.orderMetaLabels?.Result6 || "-"}
                  </span>
                </div>
              </div>
                        ...
                        <tr key={i}>
                          ...
                          <td>
                            <div className="selectInvoiceView">
                              <select
                                value={claims[i] || ""}
                                onChange={(e) =>
                                  handleClaimChange(i, e.target.value)
                                }
                              >
                                ...
                                {claim?.map((cl) => (
                                  <option
                                    key={cl.claim_reason_id}
                                    value={cl.claim_reason_id}
                                  >
                                    {cl.cl_name_en}
                                  </option>
                                ))}
                          ...
                          <td>
                            <input
                              ...
                              value={paidAmounts[i] || ""}
                              onChange={(e) =>
                                handlePaidAmountChange(i, e.target.value)
                              }
                            />
                          </td>
