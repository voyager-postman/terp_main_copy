...
const QoutationOrderCreate = ({ mode }) => {
  console.log(mode);
  ...
  //   const createTestOrder = async (mode = "order") => {
  //     const reai = details?.section5_Values?.filter(
  //       (v) => v.ITF_Name && v.QTY && v.Unit_Name
  //     );
  //     console.log("Filtered details:", reai);
  ...
  //       // Only Order has Include_claims
  //       if (mode === "order") {
  //         inputData.Include_claims = exchangeRate5 ? 1 : 0;
  //       }
  ...
  //           details: reai,
  //           Is_Recalculate: 0,
  //           Is_Quotation: mode === "quotation" ? 1 : 0,
  //         }
  //       );
  ...
  //       navigate(mode === "quotation" ? "/quotation" : "/order");
  //     } catch (e) {
  //       console.error(e);
  //       toast.error(t("tryAgain"), {
  //         autoClose: 1000,
  ...
  //     }
  //   };
  const createTestOrder = async ({ mode }) => {
    alert(mode);
    console.log("Mode is: ", mode); // check in console
    const reai = details?.section5_Values?.filter(
      (v) => v.ITF_Name && v.QTY && v.Unit_Name
    );
    if (!reai || reai.length === 0) return;
    ...
    try {
      const { data } = await axios.post(
        `${API_BASE_URL}/NewcalculateRecalculateOrder`,
        {
          input: {
            ...
            Precooling: exchangeRate4 ? 1 : 0,
            Include_claims:
              mode === "order" ? (exchangeRate5 ? 1 : 0) : undefined,
            Charge_Volume: exchangeRate1 ? 1 : 0,
            Location_name: computedState.Location_name,
          },
          details: reai,
      ...
      // ✅ toast + navigation changes by mode
      if (data.success) {
        calculateList();
        setShow(false);
        toast.success(
        ...
        await getOrdersDetails(data.data);
        navigate(mode === "order" ? "/order" : "/quotation");
      } else {
        calculateList();
        setShow(true);
        setMassageShow1(data.message);
      ...
      console.error(e);
      toast.error(t("tryAgain"), { autoClose: 1000, theme: "colored" });
    } finally {
      MySwal.close();
      setIsLoading(false);
    }
  ...
  return (
    <>
      <Card
        title={`${t(
          mode === "order" ? "orderManagement" : "quotationTestManagement"
        )} / ${t("createForm")}`}
      >
...
export default QoutationOrderCreate;
